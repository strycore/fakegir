...
GIR_PATHS = [pg`/usr/share/gir-1.0/*.gir`, pg`/usr/share/*/gir-1.0/*.gir`]
FAKEGIR_PATH = $HOME+"/.cache/fakegir"
XMLNS = "http://www.gtk.org/introspection/core/1.0"
ADD_DOCSTRINGS = 'WITHDOCS' in ${...}
...
def iter_girs():
    ...
    for gir_file in gir_files:
        ...
        gir_info = (module_name, str(gir_file))
        yield gir_info
...
def generate_fakegir():
    ...
    fakegir_repo_dir = FAKEGIR_PATH + '/gi/repository'

    if not !(test -d @(fakegir_repo_dir)):
        mkdir @(fakegir_repo_dir)

    echo '' > @(FAKEGIR_PATH + '/gi/__init__.py')
    
    echo '' > @(FAKEGIR_PATH + 'gi/repository/__init__.py')
    ...
    for module_name, gir_path in iter_girs():
        ...
        fakegir_path = FAKEGIR_PATH + '/gi/repository/' + module_name + '.py'
